    ...
    try {
      const [motRes, boiRes] = await Promise.all([
        supabase
          ...
          .select("n_moteur, code_moteur, num_serie, modele_saisi, prix_achat_moteur, etat_moteur, resa_client_moteur, nom_type_moteur")
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {detailMoteurs.map((m) => (
                            <tr key={m.n_moteur} className="hover:bg-gray-50">
                              ...
                              <td className="px-3 py-2 font-semibold">{m.nom_type_moteur || m.code_moteur || "—"}</td>
